...
| **Fail-Closed Emergency Kill Switch** | [`EmergencyKillSwitch`](file:///root/nifty-options-arbitrage/risk/kill_switch.py#L1-L70) latches upon trigger; requires explicit confirmation token `"CONFIRM_RESET"`. | Dual-trigger via REST API, Web HUD, or automated daily drawdown limit. |
...
| :--- | :--- | :--- | :--- |
| **SEV-1 (Critical)** | Daily loss $\ge ₹300$ OR Database write failure OR Unhandled process crash. | Latching Kill Switch engages; cancels open paper orders; liquidates inventory. | Review SQLite audit journal; diagnose crash dump; execute `"CONFIRM_RESET"` token only after root-cause patch. |
